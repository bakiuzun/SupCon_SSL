...
class DFCSEN12MSDataset:
    ...
    def get_patch(self, season, scene_id, patch_id, bands, window=None,mode="validation"):
        """
        ...
        """
        You may change here depending on how you have stored the data 
        """
        if mode != "sen12ms":
            ext = "validation"
            if mode == "test":
                ext = "0"
            scenedir = "{}_{}".format(sensor, ext)
            scene = "{}_{}".format(sensor, scene_id)
            filename = "{}_{}_p{}.tif".format(season, scene, patch_id)
            
            if sensor == "dfc" or mode == "test":
                patch_path = os.path.join(self.base_dir, scenedir, filename)
            else:
                patch_path = os.path.join(self.base_dir, scenedir,scenedir,filename)
        else:
    ...
    def get_s1_s2_lc_dfc_quad(self, season, scene_id, patch_id, s1_bands=S1Bands.ALL, s2_bands=S2Bands.ALL,mode="validation",lc_bands=LCBands.ALL, dfc_bands=LCBands.NONE, include_dfc=True, window=None):
        """
        ...
        s1, bounds1 = self.get_patch(season, scene_id, patch_id, s1_bands, window=window,mode=mode)
        s2, bounds2 = self.get_patch(season, scene_id, patch_id, s2_bands, window=window,mode=mode)
        lc, bounds3 = self.get_patch(season, scene_id, patch_id, lc_bands, window=window,mode=mode)
        
        if include_dfc:
            dfc, bounds4 = self.get_patch(season, scene_id, patch_id, dfc_bands, window=window,mode=mode)
